...
*
* NDVI animation for Botswana for 2022
...
var bw = ee.FeatureCollection('USDOS/LSIB_SIMPLE/2017')
...
var rgbVis = comp.map(function(img) {
  var textVis = {
    fontSize: 32, textColor: 'ffffff', 
  outlineColor: '000000', outlineWidth: 2.5, 
  outlineOpacity: 0.6 }
  ...
  var title_text = text.draw(label_title, geometryLabel, scale, {fontSize: 32})
  ...
  Map.addLayer(geometryLabel);
  var NDVI_text = text.draw(NDVI_Label, geometryLabel1, scales, {fontSize: 32});
  
  
  return img.visualize(visParams)
  .clip(bw)
  .blend(title_text).blend(date_label)
  .blend(gradientBar)
});
